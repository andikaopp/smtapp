...
class ShiftSiangKebersihan extends Page
{
    use InteractsWithForms;

    protected static string|null|\BackedEnum $navigationIcon = 'heroicon-o-cloud';
    protected string $view = 'filament.pages.shift-siang-kebersihan';
    protected static ?string $title = 'Shift Siang - Kebersihan';
    protected static ?string $navigationLabel = 'Shift Siang Kebersihan';
    ...
    {
        $userId = Auth::id();
        $kategoriId = 5; // ID Kategori untuk Pre-Shift (sesuai getFormSchema)
        ...
        $submittedChecklistCount = AktivitasShift::query()
            ...
            ->where('aktivitas_shifts.shift_id', 2) // shift_id siang
    ...
    protected function getFormSchema(): array
    {
                ...
                Section::make('Status Checklist Harian')
                    ...
                    ->schema([
                        TextEntry::make('submission_message')
                            ->label('Anda sudah berhasil melakukan submit Checklist Kebersihan Shift Siang untuk hari ini (' . now()->translatedFormat('d F Y') . '). Terima kasih!')
                    ])
                    ->aside(false)
        ...
        return [
            Repeater::make('checklists')
                ...
                ->columns(2)
                ->default(
                    Checklist::query()->where('kategori_aktivitas_shift_id', 5)
                        ->get(['id', 'todo'])
                        ->map(fn ($item) => [
                            'todo' => $item->todo,
    ...
    {
        $userId = Auth::id();
        $kategoriId = 5; // ID Kategori untuk Pre-Shift
        ...
        $submittedChecklistCount = AktivitasShift::query()
            ->where('aktivitas_shifts.user_id', $userId)
            ->whereDate('aktivitas_shifts.created_at', now()->today())
            ->where('aktivitas_shifts.shift_id', 2)
            ->join('checklists', 'aktivitas_shifts.checklist_id', '=', 'checklists.id')
            ->where('checklists.kategori_aktivitas_shift_id', $kategoriId)
            ->count();
        ...
        if ($submittedChecklistCount > 0 && $submittedChecklistCount === $requiredChecklistCount) {
            Notification::make()
                ->title('Gagal Submit')
                ->body('Anda Telah Melakukan Submit Kebersihan Shift Siang Hari Ini (Lengkap). Submit hanya diperbolehkan sekali per hari.')
                ->danger()
                ->send();
        ...
        try {
            ...
            Notification::make()
                ...
                ->body('Checklist shift siang telah berhasil disimpan.')
                ->success()
                ->send();
