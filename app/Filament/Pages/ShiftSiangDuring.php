...
class ShiftSiangDuring extends Page
{
    use InteractsWithForms;

    protected static string|null|\BackedEnum $navigationIcon = 'heroicon-o-cloud';
    protected string $view = 'filament.pages.shift-siang-during';
    protected static ?string $title = 'Shift Siang - During Shift';
    protected static ?string $navigationLabel = 'Shift Siang During';
    ...
    public function mount(): void
    {
        ...
        $submittedChecklistCount = AktivitasShift::query()
            ...
            ->where('aktivitas_shifts.shift_id', 2) // Shift_id shift siang
                ...
                Section::make('Status Checklist Harian')
                    ...
                    ->schema([
                        TextEntry::make('submission_message')
                            ->badge()
                            ->label('Anda sudah berhasil melakukan submit Checklist During Shift Siang untuk hari ini (' . now()->translatedFormat('d F Y') . '). Terima kasih!')
                            ->color('primary')
                    ])
                    ->aside(false)
    ...
    public function submit()
    {
        ...
        if ($submittedChecklistCount > 0 && $submittedChecklistCount === $requiredChecklistCount) {
            Notification::make()
                ->title('Gagal Submit')
                ->body('Anda Telah Melakukan Submit During Shift Siang Hari Ini. Submit hanya diperbolehkan sekali per hari.')
                ->danger()
                ->send();
