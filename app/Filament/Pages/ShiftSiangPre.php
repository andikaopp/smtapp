...
class ShiftSiangPre extends Page
{
    use InteractsWithForms;

    protected static string|null|\BackedEnum $navigationIcon = 'heroicon-o-cloud';
    protected string $view = 'filament.pages.shift-siang-pre';
    protected static ?string $title = 'Shift Siang - Pre Shift';
    protected static ?string $navigationLabel = 'Shift Siang Pre';
                ...
                Section::make('Status Checklist Harian')
                    ...
                    ->schema([
                        TextEntry::make('submission_message')
                            ->label('Anda sudah berhasil melakukan submit Checklist Pre Shift Siang untuk hari ini (' . now()->translatedFormat('d F Y') . '). Terima kasih!')
                    ])
                    ->aside(false)
    ...
    public function submit()
    {
        ...
        if ($submittedChecklistCount > 0 && $submittedChecklistCount === $requiredChecklistCount) {
            Notification::make()
                ->title('Gagal Submit')
                ->body('Anda Telah Melakukan Submit Pre Shift Siang Hari Ini (Lengkap). Submit hanya diperbolehkan sekali per hari.')
                ->danger()
                ->send();
