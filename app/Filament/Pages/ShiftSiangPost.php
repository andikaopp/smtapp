...
class ShiftSiangPost extends Page
{
    use InteractsWithForms;

    protected static string|null|\BackedEnum $navigationIcon = 'heroicon-o-cloud';
    protected string $view = 'filament.pages.shift-siang-post';
                ...
                Section::make('Status Checklist Harian')
                    ...
                    ->schema([
                        TextEntry::make('submission_message')
                            ->label('Anda sudah berhasil melakukan submit Checklist Post Shift Siang untuk hari ini (' . now()->translatedFormat('d F Y') . '). Terima kasih!')
                    ])
                    ->aside(false)
    ...
    public function submit()
    {
        ...
        if ($submittedChecklistCount > 0 && $submittedChecklistCount === $requiredChecklistCount) {
            Notification::make()
                ->title('Gagal Submit')
                ->body('Anda Telah Melakukan Submit Post Shift Siang Hari Ini. Submit hanya diperbolehkan sekali per hari.')
                ->danger()
                ->send();
        ...
        try {
            ...
            Notification::make()
                ...
                ->body('Checklist post shift siang telah berhasil disimpan.')
                ->success()
                ->send();
