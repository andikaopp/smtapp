...
class ActualShiftSiang extends Page
{
    ...
    protected static ?string $title = 'Actual Result Siang';
    protected static ?int $navigationSort = 5;
    protected string $view = 'filament.pages.actual-shift-siang';
    ...
    {
        $userId = Auth::id();
        $submittedTodayCount = \App\Models\ActualShiftSiang::query()
    ...
    protected function getFormSchema(): array
    {
        if ($this->hasSubmittedToday) {
            return [
                Section::make('Actual Shift Siang')
                    ->icon('heroicon-o-check-circle')
                    ->iconColor('success')
                    ->description('Pengisian Actual Result hanya dapat dilakukan sekali sehari.')
                    ->schema([
                        TextEntry::make('submission_message')
                            ->label('Submit Actual Result Siang untuk hari ini (' . now()->translatedFormat('d F Y') . ')Telah Dilakukan. Terima kasih!')
                    ])
                    ->aside(false)
                    ->heading('Pengisian Actual Result Siang Complete')
                    ->collapsed(false)
                    ->columns(1),
            ];
        }
            ...
            Grid::make(1)
                ->schema([
                    Section::make('Details')
                        ->schema([
                            TextInput::make('actual_omset_global')->numeric()->required(),
                            TextInput::make('actual_tc_global')->numeric()->required(),
    ...
    public function submit(): void
    {
        ...
        if ($submittedTodayCount > 0) {
            Notification::make()
                ->title('Gagal Submit')
                ->body('Anda Telah Melakukan Submit Actual Result Siang Hari Ini. Submit hanya diperbolehkan sekali per hari.')
                ->danger()
                ->send();
            return;
        }
        ...
        try {
            $dataToInsert[] = [
                'user_id' => Auth::id(),
                'actual_omset_global' => $targetSubmit['actual_omset_global'],
                'actual_tc_global' => $targetSubmit['actual_tc_global'],
            ...
            \App\Models\ActualShiftSiang::query()->insert($dataToInsert);

            DB::commit();

            Notification::make()
                ->title('Penyimpanan Berhasil')
                ->body('Actual Result Siang telah berhasil disimpan.')
                ->success()
                ->send();
            ...
            DB::rollBack();
            Log::error('Actual Result Siang Save Error: ' . $e->getMessage());
            Notification::make()
