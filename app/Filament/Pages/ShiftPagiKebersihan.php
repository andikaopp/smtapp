...
class ShiftPagiKebersihan extends Page
{
    use InteractsWithForms;

    protected static string|null|\BackedEnum $navigationIcon = 'heroicon-o-sun';
    protected static ?string $title = 'Shift Pagi - Kebersihan';
    protected static ?string $navigationLabel = 'Shift Pagi Kebersihan';
    protected static string|null|\UnitEnum $navigationGroup = 'Shift Pagi';
    protected static ?int $navigationSort = 1;

    protected string $view = 'filament.pages.shift-pagi-kebersihan';
    ...
    {
        $userId = Auth::id();
        $kategoriId = 4; // ID Kategori untuk Pre-Shift (sesuai getFormSchema)
    ...
    protected function getFormSchema(): array
    {
                ...
                Section::make('Status Checklist Harian')
                    ...
                    ->schema([
                        TextEntry::make('submission_message')
                            ->label('Anda sudah berhasil melakukan submit Checklist Kebersihan Shift Pagi untuk hari ini (' . now()->translatedFormat('d F Y') . '). Terima kasih!')
                    ])
                    ->aside(false)
        ...
        return [
            Repeater::make('checklists')
                ...
                ->columns(2)
                ->default(
                    Checklist::query()->where('kategori_aktivitas_shift_id', 4)
                        ->get(['id', 'todo'])
                        ->map(fn ($item) => [
                            'todo' => $item->todo,
    ...
    {
        $userId = Auth::id();
        $kategoriId = 4; // ID Kategori untuk Kebersihan Pagi
        ...
        if ($submittedChecklistCount > 0 && $submittedChecklistCount === $requiredChecklistCount) {
            Notification::make()
                ->title('Gagal Submit')
                ->body('Anda Telah Melakukan Submit Shift Pagi Kebersihan Hari Ini (Lengkap). Submit hanya diperbolehkan sekali per hari.')
                ->danger()
                ->send();
