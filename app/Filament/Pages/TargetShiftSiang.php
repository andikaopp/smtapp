...
class TargetShiftSiang extends Page
{
    ...
    {
        $userId = Auth::id();
        $submittedTodayCount = \App\Models\TargetShiftSiang::query()
    ...
    protected function getFormSchema(): array
    {
        if ($this->hasSubmittedToday) {
            return [
                Section::make('Target Shift Siang')
                    ...
                    ->schema([
                        TextEntry::make('submission_message')
                            ->label('Anda sudah berhasil melakukan submit Target Siang untuk hari ini (' . now()->translatedFormat('d F Y') . '). Terima kasih!')
                    ])
                    ->aside(false)
                    ->heading('Pengisian Target Siang Complete')
                    ->collapsed(false)
                    ->columns(1),
            ];
        }
    ...
    public function submit()
    {
        ...
        if ($submittedTodayCount > 0) {
            Notification::make()
                ->title('Gagal Submit')
                ->body('Anda Telah Melakukan Submit Target Siang Hari Ini (Lengkap). Submit hanya diperbolehkan sekali per hari.')
                ->danger()
                ->send();
        ...
        $targetSubmit = $validatedData['data']['targetShift'] ?? [];
        ...
        try {
            ...
            \App\Models\TargetShiftSiang::query()->insert($dataToInsert);

            DB::commit();

            Notification::make()
                ->title('Penyimpanan Berhasil')
                ->body('Target Siang telah berhasil disimpan.')
                ->success()
                ->send();
